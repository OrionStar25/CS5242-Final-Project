...
class Model(nn.Module):
    def __init__(self, num_vocab, num_class, hidden_size=32):
        ...
        self.rnn = nn.GRU(self.hidden_size, self.hidden_size, num_layers=3, bidirectional=True, dropout=0.2)

        self.output = nn.Linear(self.hidden_size*2, num_class)
...
def train(model, train_set, batch_size, learning_rate, num_epoch, device='cpu', model_path=None, train_validate_split=0.8):
    ...
    h_init = torch.zeros(6, batch_size, model.hidden_size)
    h = h_init.to(device)
    ...
    for epoch in range(num_epoch):
        ...
        running_vloss = 0.0
        correct = 0
        h_val = torch.zeros(6, batch_size, model.hidden_size).to(device)
        for step, vdata in enumerate(validation_loader):
            ...
            running_vloss += vloss
            predictions = torch.argmax(voutputs, dim=1)
...
def test(model, dataset, class_map, device='cpu'):
    ...
    labels = []
    with torch.no_grad():
        h_init = torch.zeros(6, 20, model.hidden_size)
        h = h_init.to(device)
...
# Accuracy: 83.12, lr=1e-3, bs=100, ne=100, num_layers=3, val_acc = 23.64
# Accuracy: 77.15 lr=1e-3, bs=100, ne=30, num_layers=5